...
$python = "C:\Users\aleja\anaconda3\envs\automate\python.exe"

# This project folder lives inside OneDrive. OneDrive's sync agent locks the
# many small intermediate files PyInstaller writes, causing random
# PermissionError failures mid-build. Build in a local (non-synced) temp
# folder instead, then copy just the final .exe back into dist\.
...
Write-Host "Installing/updating build dependencies in the 'automate' env..."
